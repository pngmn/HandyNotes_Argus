...
nodes["ArgusCore"] = {
	...
	{ coord = 66981777, npcId = 127705, questId = 48970, icon = "skull_grey", group = "rare_aw", label = _L["Mother Rosula"], search = _L["Mother Rosula_search"], loot = { { itemId = 153252, itemType = itemTypePet, speciesId = 2135, forceUnknown = false } }, note = _L["Mother Rosula_note"] },
...
nodes["ArgusSurface"] = {
	...
	{ coord = 60802080, npcId = 125388, questId = 48629, icon = "skull_grey", group = "rare_kr", label = _L["Vagath the Betrayed"], search = _L["Vagath the Betrayed_search"], loot = { { itemId = 153114, itemType = itemTypeMisc, forceUnknown = false } }, note = _L["Vagath the Betrayed_note"] },
...
nodes["ArgusMacAree"] = {
	...
	{ coord = 30124019, npcId = 126887, questId = 48709, icon = "skull_grey", group = "rare_ma", label = _L["Ataxon"], search = _L["Ataxon_search"], loot = { { itemId = 153056, itemType = itemTypePet, speciesId = 2120, forceUnknown = false } }, note = _L["Ataxon_note"] },
	{ coord = 49505280, npcId = 126913, questId = 48935, icon = "skull_grey", group = "rare_ma", label = _L["Slithon the Last"], search = _L["Slithon the Last_search"], loot = { { itemId = 153203, itemType = itemTypeMisc, forceUnknown = false } }, note = _L["Slithon the Last_note"] },
...
local function IsSourceIDFromItemLink(sourceID, itemLink)
    ...
    return tonumber(sourceItemLink:match("item:(%d+)")) == tonumber(itemLink:match("item:(%d+)"))
end
...
local function GetSourceID(itemLink)
    ...
    for i, slot in pairs(slots) do
		...
		if transmogInfo then
			if transmogInfo.appearanceID ~= nil and transmogInfo.appearanceID ~= 0 then
...
local function GetAppearanceIDFromSourceID(sourceID)
...
local function GetAppearanceID(itemLink)
    ...
    return GetAppearanceIDFromSourceID(sourceID), sourceID
end

local function IsItemSubClassIdentical(itemLinkA, itemLinkB)
    local subClassA = select(3, GetItemInfoInstant(itemLinkA))
    local subClassB = select(3, GetItemInfoInstant(itemLinkB))
...
local function canLearnTransmog(itemLink)
    ...
    local slotName = select(4, GetItemInfoInstant(itemLink))
    if slotName == "INVTYPE_TABARD" then return true end
    local sourceID = GetSourceID(itemLink)
...
local function hasTransmog(itemLink)
    ...
    local appearanceID = GetAppearanceID(itemLink)
    ...
    if sources then
        for i, source in pairs(sources) do
            ...
            if IsItemSubClassIdentical(itemLink, sourceItemLink) and source.isCollected then
                return true
            end
        end
    end
...
local function playerHasLoot( loot )
	...
	elseif ( loot["itemType"] == itemTypeTransmog ) then
		local _,itemLink = GetItemInfo( loot["itemId"] );
		if ( itemLink ~= nil ) then
			if ( hasTransmog( itemLink ) or not canLearnTransmog( itemLink ) ) then
				return true;
			else
				return false;
			end
		...
		for ii = 1, #loot do
			...
			elseif ( loot[ii]["itemType"] == itemTypeTransmog ) then
				...
				if ( itemLink ~= _L["Retrieving data ..."] ) then
					...
					elseif ( canLearnTransmog( itemLink ) ) then
						tooltip:AddDoubleLine( itemLink, string.format( _L["(itemLinkRed)"], loot[ii]["slot"] ) );
					else
						tooltip:AddDoubleLine( itemLink, string.format( _L["(itemLinkOrange)"], loot[ii]["slot"] ) );
					end
				else
...
function Argus:OnClick(button, down, mapFile, coord)
	...
	elseif button == "LeftButton" and down then
		if ( node["group"]:find("rare") ~= nil or
			node["group"]:find("invasion") ~= nil ) then
			-- find group
			LFGsearch( nil, node );
		end
    end
...
function Argus:ShowNode( mapFile, node )
    ...
    if ( C_QuestLog.IsQuestFlaggedCompleted( node["questId"] ) ) then return false end
    return true
end
